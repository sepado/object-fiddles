...
var favoriteThings = {
  ...
  movie: "Oh Brother wher art thou",
  holiday: "Easter"
};
...
//Code here
function alertObjectKeyAndValue(addObject) {
  var output = '';
  for (var i in addObject) {  // this is a FOR IN loop, MDN says not to add or del obj while using
    output += i + ": " + addObject[i] + "\n"; // \n is a new line 
  }
  alert(output);
}
...
//Code Here
function alertObjectValue(addObject) {
  for (i in addObject) {
    alert(addObject[i]);
  }
}
...
//Code Here
function alertObjectKey(addObject) {
  for (i in addObject) {
    alert(i);
  }
}
...
//Now, loop through your states object and if the states population is greater than 30K, alert 
//that state.
//Code Here
function popCheck(statesArray) {
  for(i in statesArray) {
    if (statesArray[i] > 30000) {
      alert(i);
    }
  }
}
...
// All other values are truthy
  //including "0", zero in quotes is truthy
//Code Here

//REMINDER: object.keys i .value [i]
function truthyCheck() {
  for (key in user) {
    if (!!user[key] === false) {  // !! is the NOT NOT operator, boolean value 'false', not falsey
      delete user[key];
    }
  }
}
...
//Code Here
function leaveOfAbsense(lowerCaseEmployeeName) {
  for(var i = 0; i < devMountainEmployees.length; i++) {
    if (devMountainEmployees[i] === lowerCaseEmployeeName) {
...
var user2 = {
    name: 'Sean Dougherty',
    email: 'sean.dougherty@evergreensys.com',
    password: 'abc',
    username: 'sdougherty'
};
var user3 = {
    name: 'Brendan Dougherty',
    email: 'bd@gmail.com',
    password: 'def',
    username: 'bdougherty'
};
...
//OR  - USE this version is dot notation, combining users[i].email
...
for (key in album) {
  console.log(key); //key - gets just keys
}
/* result
title1
title2
...
for (key in album) {
console.log(album[key]); //obj[key] - gets key values
}
/* result
1 min 30 sec
1 min 31 sec
...
function finder(stringTest, arrayName) {
  for(var i = 0; i < arrayName.length; i++) {
    if (arrayName[i] === stringTest) {
      return true;
    }  
  }
  return false; // the return false HAS TO BE OUTSIDE THE FOR LOOP
}
